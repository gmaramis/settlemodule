<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Create a new student or supervisor account</p>
            </div>
                    ...
                    <form method="POST" action="{{ route('users.store') }}" id="userForm">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <div class="relative">
                                    <input type="radio" id="role_student" name="role" value="student" class="sr-only peer" {{ old('role') == 'student' ? 'checked' : '' }} checked>
                                    <label for="role_student" class="flex flex-col items-center p-6 bg-white border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all duration-200">
                                        ...
                                        <h4 class="text-lg font-semibold text-gray-900">Student</h4>
                                        <p class="text-sm text-gray-600 text-center">Medical student who will participate in clinical rotations</p>
                                    </label>
                                </div>
                            </div>
                            @error('role')
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-6">
                                ...
                                <div id="student-info" class="bg-green-50 rounded-xl p-6" style="display: none;">
                                    <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div>
                                            <label for="student_institution" class="block text-sm font-medium text-gray-700 mb-2">
                                                University/Institution <span class="text-red-500">*</span>
                                            </label>
                                            <input type="text" id="student_institution" name="student_institution" value="{{ old('student_institution') }}"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition duration-200">
                                            <p class="text-xs text-gray-500 mt-1">University or medical school name</p>
                                            @error('student_institution')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                        </div>

                                        <div>
                                            <label for="student_program" class="block text-sm font-medium text-gray-700 mb-2">
                                                Program <span class="text-red-500">*</span>
                                            </label>
                                            <input type="text" id="student_program" name="student_program" value="{{ old('student_program') }}"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition duration-200">
                                            <p class="text-xs text-gray-500 mt-1">Medical program (e.g., Medicine, Nursing)</p>
                                        ...
                                        <div>
                                            <label for="student_specialization" class="block text-sm font-medium text-gray-700 mb-2">
                                                Field of Study
                                            </label>
                                            <input type="text" id="student_specialization" name="student_specialization" value="{{ old('student_specialization') }}"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition duration-200">
                                            <p class="text-xs text-gray-500 mt-1">Area of interest or specialization (optional)</p>
                                            @error('student_specialization')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    </div>
                        ...
                        <div class="flex items-center justify-end mt-8 pt-6 border-t border-gray-200">
                            <a href="{{ route('users.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-3 px-6 rounded-lg mr-4 transition duration-200">
                                Cancel
                            </a>
                            <button type="submit" class="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-3 px-8 rounded-lg transition duration-200 shadow-lg hover:shadow-xl">
                                Create User
                            </button>
                        </div>
                    </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Always show student info since we only have students now
            if (studentInfo) {
