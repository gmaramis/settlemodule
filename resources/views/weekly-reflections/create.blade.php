        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600" style="color: #4b5563 !important;">
                    Self-directed learning reflection for quality and safety improvement
                </p>
            </div>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-xl rounded-2xl overflow-hidden">
                <div class="bg-blue-600 px-6 py-8" style="background: linear-gradient(to right, #2563eb, #7c3aed) !important;">
                    <h3 class="text-2xl font-bold text-white mb-2" style="color: white !important;">Weekly Reflection Form</h3>
                    <p class="text-indigo-100" style="color: #e0e7ff !important;">
                        Reflect on your learning experiences this week to improve quality and safety in healthcare.
                    </p>
                </div>
                
                <div class="p-6 sm:p-8">
                    <form method="POST" action="{{ route('weekly-reflections.store') }}" class="space-y-8">
                        @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-gray-50 p-6 rounded-xl">
                                    ...
                                    <input id="week_start_date" type="date" name="week_start_date" 
                                        value="{{ old('week_start_date', $weekStart->format('Y-m-d')) }}" required
                                ...
                                <div class="bg-gray-50 p-6 rounded-xl">
                                    ...
                                    <input id="week_end_date" type="date" name="week_end_date" 
                                        value="{{ old('week_end_date', $weekEnd->format('Y-m-d')) }}" required
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200">
                            ...
                            <div class="bg-gray-50 p-6 rounded-xl">
                                ...
                                @if($rotations->isEmpty())
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <p class="text-sm font-medium text-yellow-800">No Clinical Rotations Available</p>
                                                <p class="text-xs text-yellow-700 mt-1">
                                                    No clinical rotations have been created yet. Please contact your administrator to create clinical rotations.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <select id="clinical_rotation_id" name="clinical_rotation_id" required
                                    ...
                                    @forelse($rotations as $rotation)
                                        <option value="{{ $rotation->id }}" {{ old('clinical_rotation_id') == $rotation->id ? 'selected' : '' }}>
                                            {{ str_replace('Rotasi ', '', $rotation->rotation_title) }}
                                        </option>
                                    @empty
                                        <option value="" disabled>No clinical rotations available</option>
                                    @endforelse
                                </select>
                                ...
                                @if($rotations->isNotEmpty())
                                    <p class="text-xs text-gray-500 mt-2">
                                        ...
                                        {{ $rotations->count() }} clinical rotation(s) available
                                    </p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-green-50 border border-green-200 p-6 rounded-xl">
                                ...
                                <textarea id="what_went_well_quality_safety" name="what_went_well_quality_safety" rows="6" required
                                    ...
                                    placeholder="Describe the positive aspects of your week related to quality and safety. What did you do well? What processes worked effectively?">{{ old('what_went_well_quality_safety') }}</textarea>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-yellow-50 border border-yellow-200 p-6 rounded-xl">
                                ...
                                <textarea id="what_could_do_better" name="what_could_do_better" rows="6" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 resize-none"
                                    placeholder="Reflect on areas where you could have performed better. What would you do differently next time?">{{ old('what_could_do_better') }}</textarea>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-blue-50 border border-blue-200 p-6 rounded-xl">
                                ...
                                <textarea id="what_learned_safe_healthcare" name="what_learned_safe_healthcare" rows="6" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 resize-none"
                                    placeholder="Share your key learnings about safe healthcare systems, patient safety principles, quality improvement, or any safety protocols you encountered.">{{ old('what_learned_safe_healthcare') }}</textarea>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-purple-50 border border-purple-200 p-6 rounded-xl">
                                ...
                                <textarea id="goals_for_next_week" name="goals_for_next_week" rows="4" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 resize-none"
                                    placeholder="Set specific, achievable goals for the upcoming week. Focus on quality, safety, and learning objectives.">{{ old('goals_for_next_week') }}</textarea>
